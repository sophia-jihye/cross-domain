...
finetune_raw_dirs = glob('/media/dmlab/My Passport/DATA/cross-domain/finetune_using_post-trained_unk-specific/source=*_raw_post=*')
parent_save_dir = '/media/dmlab/My Passport/DATA/cross-domain/test_using_post-trained_unk-specific'
...
if __name__ == '__main__':
    ...
    for finetune_raw_dir in finetune_raw_dirs:
        domain = os.path.basename(finetune_raw_dir).split('_')[0].replace('source=','')
        test_domain = os.path.basename(finetune_raw_dir).split('_')[2].replace('post=','').replace(domain, '').replace('&', '')
...
#         finetune_unk_dir = finetune_raw_dir.replace('raw', 'unk')
