...
post_trained_dir_format = '/media/dmlab/My Passport/DATA/cross-domain/post-train/{}_{}'
...
parent_save_dir_format = '/media/dmlab/My Passport/DATA/cross-domain/finetune_upper_limit_using_post-trained_{}'
...
if __name__=='__main__':
    ...
    for pt_mode in ['unk', 'raw']:
        for train_filepath in train_filepaths:
            for mode in ['unk', 'raw']:
                domain = os.path.basename(train_filepath).split('_')[0]
                model_name_or_dir = post_trained_dir_format.format(domain, pt_mode)
                kfold_idx = os.path.basename(train_filepath).split('_')[1].replace('k=', '')
                save_dir = os.path.join(parent_save_dir_format.format(pt_mode), '{}_{}_k={}'.format(domain, mode, kfold_idx))
